...
版本通过 git tag 管理，可随时回滚：`git checkout <tag>`

## [v0.3.0] - 2026-06-30

### 版本管理体系与 Bug 修复

建立项目版本控制基线，修复 AI 验证层的关键 Bug，补全工程化基建。

#### 新增功能

- **版本号管理** (`src/config.py`)
  - 新增 `__version__` / `__version_info__` / `__app_name__` / `__app_name_en__`
  - 采用 SemVer 语义化版本号
  - `main.py` 启动时显示版本号和应用名称

- **Git 标签版本控制**
  - 为 v0.2.0 打基线标签（`git tag v0.2.0`），支持随时回滚
  - 后续每个版本对应一个 git tag

- **项目文档** (`README.md`)
  - 完整的项目说明：功能、技术栈、快速开始、项目结构
  - 版本管理说明（查看版本、回滚方法）
  - 开发路线图

#### Bug 修复

- **OCRValidator 增加 `failed_reasons` 字段** (`src/ai_guard/ocr_validator.py`)
  - 问题：`validate()` 方法不返回 `failed_reasons`，但 `demo_web.py` 引用了该字段，导致验证未通过时原因说明永远为空
  - 修复：`validate()` 现在返回 `failed_reasons` 列表，包含每个失败字段的具体原因
  - 失败原因示例：`"取餐码格式无效: '!@#$'"`、`"OCR平均置信度过低: 0.35 (阈值: 0.6)"`

- **Web 演示使用验证后的信息播报** (`tools/demo_web.py`)
  - 问题：验证通过时 `format_speech()` 接收原始 `info`，可能包含未通过验证的无效字段
  - 修复：改为传入 `validation["validated_info"]`，确保只播经验证的字段

#### 变更

- `main.py` 启动语音从"盲找助手已启动"改为"慧眼 SmartEye已启动"
- `main.py` 初始化日志增加版本号显示

#### 版本控制说明

- `v0.2.0` 标签标记了 AI 验证层完成时的代码状态
- `v0.3.0` 标签标记了版本管理体系建立和 Bug 修复后的状态
- 回滚命令：`git checkout v0.2.0`（回滚到上一版本）
