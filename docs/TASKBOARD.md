...
- [x] **弱光环境自动开手电**：OCR 识别率受光线影响，昏暗环境下系统主动提示并尝试打开手电
  - 新增 `LuminanceDetector`（8×8 网格采样 + BT.601 加权，< 5ms / 720p）
  - 新增 `TorchController`（`CameraController.setFlashMode()` 幂等封装，硬件异常捕获）
  - HomeScreen 每帧分析亮度，`< 40/255` 触发「光线较暗」语音 + 自动开手电
  - 8s 冷却防止「光线较暗」语音刷屏，30s 冷却防止硬件不支持时循环提示
  - 3 段新语音：`luminance_dim.mp3` / `torch_on.mp3` / `torch_failed.mp3`
  - 22 个新单测（117 → 139）；v0.8.0 将切换 YUV420 Y 平面流进一步降耗
