...
>
> **接手顺序**：先读 [EVOLUTION.md](./docs/EVOLUTION.md) 了解产品演进全貌与关键决策背景 → 再读 [TASKBOARD.md](./docs/TASKBOARD.md) 看当前冲刺与待办 → 再读本文件 → 最后读 [PROJECT.md](./PROJECT.md) 了解架构决策与已知问题。
...
- ❌ **禁止静默失败**：任何异常路径都必须有语音播报解释发生了什么、用户该怎么办（详见 §5.3 NSF 红线）

### 5.3 语音反馈规范（强制）

> **核心红线：视障工具绝不静默失败**（No Silent Failure, NSF）
> 
> 视障用户无法通过屏幕看到任何错误信息。任何异常路径（权限拒绝、硬件不支持、初始化失败、网络中断、OCR 识别失败等）都必须有**语音播报**说明发生了什么、下一步该怎么办。**日志 / Toast / Snackbar / 颜色变化都不能替代语音**。

#### 5.3.1 必须播报语音的场景（不完整列表）

| 场景 | 必播内容 | 必走路径 |
|------|---------|----------|
| 摄像头权限被拒 | "未获得摄像头权限，请开启" | `TtsService.speakCameraPermissionDenied()` |
| 摄像头权限永久拒绝 | "请到系统设置开启摄像头权限" | `speakCameraPermissionPermanentlyDenied()` → 跳转设置 |
| 手电硬件不支持 | "自动打开手电失败，请手动开启" | `speakTorchFailed()` |
| OCR 初始化失败 | "文字识别服务未就绪" | `TtsService` 新增方法（待补） |
| 相机初始化 3 次失败 | "摄像头不可用，请检查权限" | `_initCameraWithRetry()` 失败分支 |
| 静音 / 媒体音量为 0 | "请打开媒体音量" | 检测 AudioManager（v0.8.0 计划） |
...
**反面例子**（禁止）：
- `print('error: $e')` 后无任何语音
- `ScaffoldMessenger.showSnackBar(...)` 视障用户看不到
- `setState(() => _isError = true)` 仅靠 UI 变化
- 写 FileLogger 后沉默（视障用户不知道有日志可看）
...
- [ ] 所有交互元素有 Semantics
- [ ] 所有用户操作有语音反馈
- [ ] **异常路径 100% 有语音**：检查所有 `catch` / `if (status == X)` 失败分支，**没有任何静默失败**（无 print 替代、无 UI 替代、无 FileLogger 替代）
- [ ] 新增异常场景的语音已录制并注册到 `TtsService`

### 9.3 隐私与安全
- [ ] 没有上传原图或相机帧到任何服务器
- [ ] 没有收集设备信息 / 位置信息
- [ ] APK 仅含 `arm64-v8a`（无其他 ABI）

### 9.4 文档与版本规范（发版时必检）
- [ ] `CHANGELOG.md` 版本描述严谨专业，无口语化表达、无占位符、无随意批注
- [ ] `CHANGELOG.md` 版本条目完整，每个已发布版本均有条目，无遗漏
- [ ] `CHANGELOG.md` 版本章节严格按从新到旧降序排列
- [ ] 每条变更包含「改了什么+为什么改+影响是什么」三要素
- [ ] `VERSION.md` 版本历史表格严格从新到旧降序排列，无重复条目、无缺失版本
- [ ] `VERSION.md` 版本历史表格「说明」列已填写完整，无「预留」「待补充」等占位文字
- [ ] `pubspec.yaml` 版本号与 git tag 一致
