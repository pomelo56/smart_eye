...
void main() {
  group('MealCode', () {
    test('serializes and deserializes correctly', () {
      final code = MealCode(code: '#15', recognizedAt: DateTime(2026, 6, 30, 12, 0));
    ...
    test('time description shows "刚刚" for recent codes', () {
      final code = MealCode(code: '#15', recognizedAt: DateTime.now().subtract(const Duration(seconds: 30)));
      expect(code.timeDescription, equals('刚刚'));
    });

    test('time description shows minutes for older codes', () {
      final code = MealCode(code: '#15', recognizedAt: DateTime.now().subtract(const Duration(minutes: 5)));
